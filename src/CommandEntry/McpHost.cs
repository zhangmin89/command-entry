...
internal static class McpHost
{
    ...
    private static IList<Tool> Tools()
    {
        static JsonObject String() => new() { ["type"] = "string" };
        static JsonObject Array() => new() { ["type"] = "array", ["items"] = String() };
        var start = new JsonObject();
        foreach (string name in new[] { "operation", "program", "workdir", "script", "language", "parameters_file", "stdin_file", "encoding", "previous_execution" }) start[name] = String();
