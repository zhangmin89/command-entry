...
internal sealed class McpClient : IDisposable
{
    ...
    internal McpClient(string policy, string? binding = null, string? executable = null)
    {
        ...
        Process = System.Diagnostics.Process.Start(WindowsProcess.StartInfo(executable ?? TestEnvironment.Server, args, TestEnvironment.Root))!;
    ...
    internal JsonObject Rpc(string method, JsonObject parameters)
    {
        ...
        while (true)
        {
            ...
            Assert.False(message.ContainsKey("error"), message.ToJsonString());
    ...
    internal JsonObject Call(string name, JsonObject arguments)
    {
        ...
        Assert.False(result["isError"].IsTrue(), result.ToJsonString());
    ...
    internal (int Exit, string Error) CloseInput()
    {
        ...
        Assert.True(Process.WaitForExit(10000), "MCP server did not exit after stdin closed");
    ...
    public void Dispose()
    {
        ...
        {
            Process.StandardInput.Close();
            if (!Process.WaitForExit(20000)) { Process.Kill(); Assert.True(Process.WaitForExit(10000)); }
        }
...
{
    internal string DirectoryPath { get; } = Path.Combine(TestEnvironment.Root, ".codex-command-records", "csharp-test-" + Guid.NewGuid());
    ...
    internal Fixture()
    {
        ...
        Policy = TestEnvironment.TemplatePolicy();
        ...
        {
            ["probe"] = new JsonObject { ["kind"] = "native", ["path"] = TestEnvironment.ProbeExe },
            ["server"] = new JsonObject { ["kind"] = "native", ["path"] = TestEnvironment.Server },
    ...
    {
        using var process = Process.Start(WindowsProcess.StartInfo(executable, args, TestEnvironment.Root))!;
        var stdout = process.StandardOutput.ReadToEndAsync(); var stderr = process.StandardError.ReadToEndAsync();
        if (input is not null) process.StandardInput.Write(input);
        process.StandardInput.Close();
        Assert.True(process.WaitForExit(60000), "child did not exit");
