...
def main():
    ...
    from tests import test_csharp_migration as migration
    migration.EXE = root / 'artifacts/command-entry-win-x64/CommandEntry.exe'
    case = migration.CSharpMigrationTests()
    case.setUp()
    try:
        reference = migration.Server(case.policy_file)
        observations = []
        for name, data in (
            ...
            for count in (1, 2):
                ...
                observations.append({'case': name, 'count': count,
                    ...
                    'csharp': case.client.call('read_text', **form)})
        report = case.directory / 'read-text-parity.json'
